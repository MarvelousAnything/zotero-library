rgb = {
	...
	init: function () {
		...
		// PODCAST PAGE
		// if ($('body').hasClass('g-new-podcast-page')) {
		// 	rgb.rgbFeaturesSpecialChangeHeaderBG();
		// }
		// single
		if ($('body.single').length) {
			...
			if ( ( $('body').hasClass('single-features-special') || $('body').hasClass('header-transparent') ) && !$('body').hasClass('single-spotlight') && !$('body').hasClass('mobile') ) {
		...
		rgb.rgbGetUserOperatingSystem();

		if(location.origin == 'https://www.timesofisrael.com' || location.origin == 'http://dev.timesofisrael.com'){
			rgb.rgbOmnyPlayer();

		}
		if ($('.daily-briefing-widget')) {
			rgb.rgbSetDailyBriefingHeight();
		}
	},
	rgbWeather: function () {
	...
	rgbFeaturesSpecialChangeHeaderBG: function () {
		...
		// {TOI} Change community banner display settings
		function showCommunityBanner(){
			if($('.join-community-banner').length){
			var banner = $('.join-community-banner'),
				header_color = $('.header').css('background-color'),
			    opacity = 0;
			if(header_color === "rgb(30, 30, 30)"){
				opacity = 1
			}else {
				opacity = 0;
			}
				banner.css({"opacity" : + opacity ,"transition": "opacity .3s ease-in-out"});
			}
		}
		...
		$.each(scrollAreas, function (x, v) {
			$(v).scroll(function () {
				$('.sticky-sidebar').each(function () {
					...
					var startStickySidebar = (block.outerHeight() - stickySidebar.outerHeight() >= 110);
    ...
    if (body.hasClass('small-header')) {
      ...
      var stickyStart = 50;
      var stop = false;
      var $new_header_toi = $('.new-header-toi');
      $(window).scroll(function () {
        var scrollTop = $(window).scrollTop();

        if (scrollTop >= stickyStart) {
          if (!stop) {
			  body.addClass('hide-community-banner');
            stop = true;
          }
        } else {
          if (stop) {
			  body.removeClass('hide-community-banner');
            $('.header-bottom').removeAttr("style");
			  $new_header_toi.removeClass('close');
			  $new_header_toi.removeClass('open');
            stop = false;
          }
        }
      });
		...
		if (!body.hasClass('small-header') && !body.hasClass('tag-nxtgen')) {
			var header = $('.header');
		...
		$(document).on('click', '.menu-control', function () {
			...
			var header = $('.header');
			// start here
			if ($('.header').hasClass('new-header-toi')) {
				// hide search bar

				if ($new_header_toi.hasClass('open') ) {
					$new_header_toi.removeClass('open');
					$new_header_toi.addClass('close');
					// $('.header-bottom').slideUp(function () {
	...
	rgbHomeLoadSections: function () {
		...
		if ($('.data-sections').length) {
			var sectionsAvailable = $('.data-sections').data('sections').toString();
	...
	rgbSubMenuDropDown: function () {
		...
		$(document).on('click', '.dropdown-btn:not(.detected-device)', function (event) {
					...
					$.ajax({
						...
						myThisSection.find('.items').fadeOut('fast', function () {
							$(this).html(html).fadeIn('fast', function () {
								...
								// {TOI} FR | Replace ad tags with video tags
								if (myThisSection.find('#div-gpt-ad-1615294391209-0').length) {
									googletag.pubads().refresh([window.adSlot_1615294391209_0]);
								}
	...
	rgbLoadMoreClick: function () {
		$(document).on('click', '.load-more a', function (event) {
			...
			var is_load_more = myThis.data('is_load_more') ? myThis.data('is_load_more') : 0;
	...
	rgbMailchimpSubmitForm: function () {
		$(document).on('click', '.newsletter .submit, .newsletter-new .submit', function (event) {
			event.preventDefault();
			var newHeaderLocation = $(this).parents('.newsletter-new');
			var location = $(this).parents('.newsletter');
			if (!location.length && newHeaderLocation.length) {
				location = newHeaderLocation;
			}
			...
			if (email == ' ' || rgb.rgbIsValidEmailAddress(email) === false) {
				errorBox.html('<span>The email address is invalid or missing.<br>Please try again.</span>');
				errorBox.fadeIn();
			} else {
				errorBox.html('<span></span>');
				errorBox.hide();
				...
				jQuery.ajax({
					...
					if (msg == 'Success') {
						if(website =='timesofisrael'){
							fetch(`https://crm.timesofisrael.com/dripCampaign?member=${btoa( email )}`, { method: 'POST'}) .then( response => response.json() ) .then( member => { console.log(member) } );
						}

						var html = '<div class="almost thank-you">';
						html += '<div class="big">Thank you.</div>';
						if(website == 'ajd'){
							html += '<p>You\'re now subscribed to The Weekly Edition.</p>';
						}else if(website == 'realestate') {
							html += '<p>You\'re now subscribed to Real Estate Israel.</p>';
						}else{
							html += '<p>You\'re now subscribed to The Daily Edition.</p>';
						}
						html += '</div>';
						location.html(html);
						...
						var html = '<div class="almost thank-you">';
						html += '<div class="big">متشکرم</div>';
						html += '<p>شما اکنون در خبرنامه مشترک شده اید</p>';
						html += '</div>';
						location.html(html);
						ga('send', 'event', 'signup', 'FA newsletter signup', from, 1);
					} else {
						errorBox.find('span').html(msg);
						location.css('padding-bottom', '14px');
						errorBox.fadeIn();
					}
				});
			}
								...
								$.ajax({
									...
									if ($('body').hasClass('AJN')) {
										...
										if (!rgb.rgbIsMobile()) {
											...
											if(website == 'jewishnews.timesofisrael.com' || website == 'jewishndev.timesofisrael.com' || website == 'dev.jewishnews.co.uk' || website == 'www.jewishnews.co.uk'){
												rgb.rgbLoadBanner(currentArticleIdLoad, articleIdLoad, 'BTF_SKY_1', [[300, 1050], [300, 600], [300, 250]], '.banner.BTF_SKY_1');
											}else{
												rgb.rgbLoadBanner(currentArticleIdLoad, articleIdLoad, 'BTF_SKY_1', [300, 600], '.banner.BTF_SKY_1');
											}
											rgb.rgbLoadBanner(currentArticleIdLoad, articleIdLoad, '1615294391209-0', [1, 1], '.banner.1615294391209-0');
					...
					if (array.length > 0 && stratToChangeMetas == true) {
						...
						if ($('#' + currentArticleId).offset().top <= scrollTop) {
							if (!doChangesStopper) {
								...
								if (location.host.indexOf("jewishnews") === -1 && location.host.indexOf("jewishndev") === -1 || location.host !== 'dev.jewishnews.co.uk' || location.host !== 'www.jewishnews.co.uk') {
	...
	rgbArticleLiveblogPopup: function () {
		$(document).on('click', '.article-popup-close, .wrap-article.liveblog.article-popup', function (e) {
			...
			rgb.rgbLiveblogAppPostMesseage('top');
		});
		if (
			$('.article-popup').hasClass('active') &&
			(navigator.userAgent.indexOf('ios app') > -1 ||
			navigator.userAgent.indexOf('android app') > -1)
		) {
			var oldScrollTop = $('.article-popup.active .article-content.sticky-sidebar-relative').scrollTop();
			$('.article-popup.active .article-content.sticky-sidebar-relative').scroll(function () {
				var currentScrollTop = $(this).scrollTop();
				if (oldScrollTop == 0 && currentScrollTop > 0) {
					rgb.rgbLiveblogAppPostMesseage('down');
				} else if(oldScrollTop > 0 && currentScrollTop == 0){
					rgb.rgbLiveblogAppPostMesseage('top');
				}
	...
	rgbGoogleAnalyticsPageView: function (articleId, url, title, index) {
		...
		if ($('#' + articleId).data('tags')) {
			topics = $('#' + articleId).data('tags');	
			if (topics !== '') {
				topics += ' ';
			}		
		}

		if ($('#' + articleId).data('writers')) {
			writers = $('#' + articleId).data('writers');	
			if (writers !== '') {
				writers += ' ';
			}			
		}
		if(articleId != false && writers == ''){
			writers = 'No writer';
		}
		...
		if (topics !== '') {
			ga('partnerTracker.set', {'dimension1': topics});
		}
		if (writers !== ''){
			ga('partnerTracker.set', {'dimension2': writers});
		}
		...
		// Private Partner pageview
		if ($('body').data('google_analytics_ptivate_partner_id')) {
	...
	rgbLoadBanner: function (currentArticleId, articleId, slotName, slotSize, slotClass, slotAccount, slotOldName) {
		...
		var account = (typeof (slotAccount) != "undefined" && slotAccount !== null) ? slotAccount : 22215292831;
		if ($('#' + slotId).length == 0 && $(slotClass).length) {
			$('#' + currentArticleId + ' ' + slotClass).attr('id', slotId);
	...
	rgbLiveblogAppPostMesseage: function (msg) {
		if (msg == 'top') {
			if (navigator.userAgent.indexOf('ios app') > -1) {
				window.webkit.messageHandlers.IOS.postMessage({"key":"onPopupScrolledToTop"});
			}
			if (navigator.userAgent.indexOf('android app') > -1) {
				Android.onPopupScrolledToTop();
			}
		} else if(msg=='down'){
			if (navigator.userAgent.indexOf('ios app') > -1) {
				window.webkit.messageHandlers.IOS.postMessage({"key":"onPopupScrolledDown"});
			}
			if (navigator.userAgent.indexOf('android app') > -1) {
				Android.onPopupScrolledDown();
			}
		}
	},
	rgbCommunityMenu: function () {
		...
		var is_fr = false;
		var status = rgb.getUserStatus();
		var prefix = '';
		var txt1 = 'My profile';
		var txt2 = 'Community page';
		var txt3 = 'Support';
		var txt4 = 'Sign out';
		...
		communityLoggedMenu += '<li><a href="https://crm.timesofisrael.com/'+ prefix +'profile"><span>'+ txt1 +'</span></a></li>';
		if (status == 'active' && !is_fr) {
			communityLoggedMenu += '<li><a href="https://timesofisrael.com/community/"><span>'+ txt2 +'</span></a></li>';
		}
		communityLoggedMenu += '<li><a href="https://crm.timesofisrael.com/'+ prefix +'support"><span>'+ txt3 +'</span></a></li>';
		communityLoggedMenu += '<li><a href="https://crm.timesofisrael.com/'+ prefix +'sign-out"><span>'+ txt4 +'</span></a></li>';
	...
	getUserStatus: function() {
		let crmUser = decodeURIComponent( rgb.getCookie('CrmUser') );
	  if( crmUser ) {
	    try {
	...
	rgbGetUserOperatingSystem: function () {
		...
		if (navigator.userAgent.indexOf('rgbmedia-app')==-1 && navigator.userAgent.indexOf('mobiletoi')==-1) {
			if((device == 'Android' || device == 'iOS') && !isTablet() && ((typeof window.orientation !== "undefined") || (navigator.userAgent.indexOf('IEMobile') !== -1)) ){
				var appLink = $('.app-item li[data-os="' + device + '"] a').attr('href');
				$('.app-item .dropdown-btn').addClass('detected-device');
				$('.app-item .dropdown-btn').attr('href', appLink);
			} else {
				$('.app-item li').show();
			}
		} else {
	...
	rgbSetDailyBriefingHeight: function () {
        var widgetWidth = $('.daily-briefing-widget').width();
        $('.daily-briefing-widget').height(`${widgetWidth}px`);  
	},
	rgbOmnyPlayer: function(){
		if( $('.omny-wrapper iframe:not(.playerjs_loaded)') .length >0 && typeof playerjs != 'undefined'){
			var sources = [];
			...
			function remove_source(src){
				Array.prototype.remove = function() {
					var src, a = arguments, L = a.length, ax;
					while (L && this.length) {
						src = a[--L];
						while ((ax = this.indexOf(src)) !== -1) {
							this.splice(ax, 1);
						}
					}
					return this;
			...
			// if (!$('body').hasClass('mobile')) {
				var player;
				// playerjs can only be initialized after the iframe has loaded
				$('.omny-wrapper iframe:not(.playerjs_loaded)').each(function() {
					$(this).addClass('playerjs_loaded');
					// assign playerjs to the iframe
					player = new playerjs.Player(this);
					player.on('play', function () {
						var src = $(this)[0].elem.src;
						// $('.g-new-podcast-post-item').removeClass('play');
						// $($(this)[0].elem).parents('.g-new-podcast-post-item').addClass('play');
						add_source(src);
					...
					player.on('pause', function () {
						var src = $(this)[0].elem.src;
						remove_source(src);
						console.log('remove_source');
						start_timer();
					});
					// when the player is ended
...
if (location.host.indexOf("jewishnews") === -1 && location.host.indexOf("jewishndev") === -1) {}
